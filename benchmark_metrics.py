...
from NSGA_II_functions import fast_non_dominated_sort
...
def compute_spacing(F):
    ...
    return np.sqrt(np.sum((d - np.mean(d)) ** 2) / (len(F) - 1))
...
def compute_spread(F, extreme_1, extreme_2):
    ...
    Inputs: 
        F: array of fitness values for the obtained front
        extreme_1, extreme_2: the two extreme solutions of the true Pareto
                              front. 
    '''
...
def run_benchmark(seeds, N, data, T_max, p_c, p_m, ref_point, 
