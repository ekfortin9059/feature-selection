...
import nsga_functions as nsga2
...
#%%
def my_NSGA_II(N, data, n_generations, p_c, p_m, seeding_prop, ones_prop, importances, seed=None):
    '''
    NSGA-II Main Loop
    '''
    ...
    parent_pop.initialize(data.n, importances, seeding_prop, ones_prop, rng)
...
#%% Single run of my nsga-ii alg
import time
start = time.time()
importances = SelectFromModel(estimator=LinearRegression()).fit(data.X,data.y).estimator_.coef_[0]
pop = my_NSGA_II(N, data, T_max, p_c, p_m, seeding_prop, ones_prop, importances, seed=seed)
...
hv_pymoo = HV(ref_point=ref_point)(res.F )
print(f"Hypervolume (pymoo): {hv_pymoo:.4f}")
 
plt.figure(figsize=(7, 5))
plt.scatter(res.F [:, 1], -res.F[:, 0], facecolors='none', edgecolors='orange')
plt.title("Objective Space - Pymoo NSGA3")
plt.show()
...
plt.figure(figsize=(7, 5))
plt.scatter(res.F [:, 1], -res.F[:, 0], facecolors='none', edgecolors='black')
plt.title("Objective Space - Pymoo SPEA2")
plt.show()
